...
MSTR (ahora "Strategy Inc.", mismo ticker), en Bitget.

Reglas CriptoNorber (15/09): el capital cargado con "capital_total" es el
"Cargador 1", con 2 balas — capital/2 por bala, SIN cambios en la fórmula
(la bala real DS-01 ya se cargó así). Un Cargador 2 (también 2 balas) queda
como concepto para más adelante, sin programar todavía — mismo criterio que
en MS. Cada bala se carga 20% como monto de trade (con el apalancamiento
configurado, informativo) y 80% como margen adicional extra, para alejar la
liquidación.
...
def load_ds_state():
    if os.path.exists(DS_STATE_FILE):
        ...
        state = {}
    state.setdefault("capital_total", CAPITAL_TOTAL_DEFAULT)
    state.setdefault("balas_usadas", 0)
    state.setdefault("log", [])
    state.setdefault("_previous_snapshot", None)
    state.setdefault("contador_operaciones", 0)  # para los ids DS-01, DS-02, ...
    return state
...
def _siguiente_id_operacion(state):
    """Igual que en saylor_bot.py: el contador vive dentro del state, así que
    deshacer una operación (que restaura el state completo) también revierte
    el contador — el próximo id real reutiliza el número liberado."""
    state["contador_operaciones"] = state.get("contador_operaciones", 0) + 1
    return f"DS-{state['contador_operaciones']:02d}"
...
def deshacer_ultima_bala(state):
    """Devuelve la entry deshecha (con su id) o None si no había nada."""
...
def format_status_message(state, precio_actual=None):
    ...
    for e in state.get("log", []):
        ...
        lines.append(
            f"— {e.get('id', '?')} (bala {e['bala']}): entrada ${e['precio_entrada']:,.2f} | "
...
def format_registro_message(result):
    ...
    e = result["entry"]
    return (
        f"✅ {e.get('id', '?')} (bala {e['bala']}) de DS registrada.\n"
...
def handle_command(text):
    ...
    if lower == "/ds_deshacer":
        state = load_ds_state()
        deshecho = deshacer_ultima_bala(state)
        if deshecho:
            save_ds_state(state)
            return f"↩️ Deshecho {deshecho.get('id', '?')} — DS volvió al estado anterior a esa bala."
